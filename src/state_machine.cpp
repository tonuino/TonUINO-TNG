...
{
  if (currentValue != 0 && preview && not previewStarted && previewTimer.isExpired() && not SM<SMT>::mp3.isPlaying())
  {
...
void ChFolder::react(button_e const &b)
{
  ...
  if ((b.b == buttonRaw::pause) && (currentValue != 0))
  {
    ...
    if ((folder.mode == mode_t::hoerspiel_vb) || (folder.mode == mode_t::album_vb) || (folder.mode == mode_t::party_vb))
    {
...
void Admin_SimpleSetting::entry()
{
  ...
  numberOfOptions = type == maxVolume ? 30 - settings.minVolume : type == minVolume ? settings.maxVolume - 1
                                                              : type == initVolume  ? settings.maxVolume - settings.minVolume + 1
                                                              : type == eq          ? 6
                                                                                    : 0;
  startMessage = type == maxVolume ? mp3Tracks::t_930_max_volume_intro : type == minVolume ? mp3Tracks::t_931_min_volume_into
                                                                     : type == initVolume  ? mp3Tracks::t_932_init_volume_into
                                                                     : type == eq          ? mp3Tracks::t_920_eq_intro
                                                                                           : mp3Tracks::t_0;
  messageOffset = type == maxVolume ? static_cast<mp3Tracks>(settings.minVolume) : type == minVolume ? mp3Tracks::t_0
  ...
  currentValue = type == maxVolume ? settings.maxVolume - settings.minVolume : type == minVolume ? settings.minVolume
...
void Admin_StandbyTimer::react(button_e const &b)
{
  ...
  if ((b.b == buttonRaw::pause) && (currentValue != 0))
  {
    ...
    {
    case 1:
      settings.standbyTimer = 5;
      break;
    case 2:
      settings.standbyTimer = 15;
      break;
    case 3:
      settings.standbyTimer = 30;
      break;
    case 4:
      settings.standbyTimer = 60;
      break;
    case 5:
