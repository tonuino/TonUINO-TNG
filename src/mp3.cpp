...
void Mp3Notify::PrintlnSourceAction(DfMp3_PlaySources source, const __FlashStringHelper *action)
{
  if (source & DfMp3_PlaySources_Sd)
    LOG(mp3_log, s_debug, F("SD Karte "), action);
  if (source & DfMp3_PlaySources_Usb)
    LOG(mp3_log, s_debug, F("USB "), action);
  if (source & DfMp3_PlaySources_Flash)
    LOG(mp3_log, s_debug, F("Flash "), action);
}
...
Mp3::Mp3(const Settings &settings)
    : Base{softwareSerial}, softwareSerial{dfPlayer_receivePin, dfPlayer_transmitPin}, settings{settings}
#endif /* DFPlayerUsesSoftwareSerial */
#ifdef DFPlayerUsesHardwareSerial
      Mp3::Mp3(const Settings &settings)
    : Base{dfPlayer_serial}, settings{settings}
