...
Buttons::Buttons(const Settings &settings)
    ...
    : buttonPause(buttonPausePin, buttonDbTime, buttonPinIsActiveLow, buttonPinIsActiveLow), buttonUp(buttonUpPin, buttonDbTime, buttonPinIsActiveLow, buttonPinIsActiveLow), buttonDown(buttonDownPin, buttonDbTime, buttonPinIsActiveLow, buttonPinIsActiveLow)
#ifdef FIVEBUTTONS
      ,
      buttonFour(buttonFourPin, buttonDbTime, buttonPinIsActiveLow, buttonPinIsActiveLow), buttonFive(buttonFivePin, buttonDbTime, buttonPinIsActiveLow, buttonPinIsActiveLow)
#endif
      ,
      settings(settings)
{
...
buttonRaw Buttons::getButtonRaw()
{
  ...
  readButtons();
  if ((buttonPause.pressedFor(buttonLongPress) || buttonUp.pressedFor(buttonLongPress) || buttonDown.pressedFor(buttonLongPress)) && buttonPause.isPressed() && buttonUp.isPressed() && buttonDown.isPressed())
  {
...
buttonCmd Buttons::getButtonCmd(buttonRaw b)
{
  ...
  case buttonRaw::pauseLong:
    ...
    break;
  case buttonRaw::up:
    ret = (!settings.invertVolumeButtons) ? buttonCmd::next : buttonCmd::volume_up;
    break;
  case buttonRaw::upLong:
    ret = (!settings.invertVolumeButtons) ? buttonCmd::volume_up : buttonCmd::next;
    break;
  case buttonRaw::down:
    ret = (!settings.invertVolumeButtons) ? buttonCmd::previous : buttonCmd::volume_down;
    break;
  case buttonRaw::downLong:
    ret = (!settings.invertVolumeButtons) ? buttonCmd::volume_down : buttonCmd::previous;
    break;
  case buttonRaw::allLong:
    ret = buttonCmd::admin;
    break;
#ifdef FIVEBUTTONS
  case buttonRaw::four:
...
{
  return not buttonPause.isPressed() && not buttonUp.isPressed() && not buttonDown.isPressed();
}
