#ifndef SRC_TROVALIBRE_FIXTURE_HPP_
#define SRC_TROVALIBRE_FIXTURE_HPP_
...
class trovalibre_fixture: public ::testing::Test {
public:
  trovalibre_fixture()
  : initializer{}
  ...
  Mp3&       getMp3     () { return trovaLibre.getMp3     (); }
  Commands&  getCommands() { return trovaLibre.getCommands(); }
  Settings&  getSettings() { return trovaLibre.getSettings(); }
  Chip_card& getChipCard() { return trovaLibre.getChipCard(); }
  MFRC522&   getMFRC522 () { return getChipCard().mfrc522; }
  Modifier&  getModifier() { return trovaLibre.getActiveModifier(); }
  ...
  void goto_idle() {
    ...
    if (SM_trovaLibre::is_in_state<StartPlay<Play>>()) {
    ...
    if (SM_trovaLibre::is_in_state<Pause>()) {
    ...
    if (SM_trovaLibre::is_in_state<Play>()) {
      ...
      // loop for all tracks
      while (not SM_trovaLibre::is_in_state<Idle>()) {
        getMp3().end_track();
        execute_cycle();
        execute_cycle();
      }
      ...
      int counter = 10;
      while (not SM_trovaLibre::is_in_state<Idle>() && --counter) {
        getMp3().end_track();
        execute_cycle();
      }
      EXPECT_TRUE(SM_trovaLibre::is_in_state<Idle>());
...
#endif // SRC_TROVALIBRE_FIXTURE_HPP_
